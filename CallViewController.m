...
//            connectStr = @"Direct";
//        }
        self.statusLabel.text = [NSString stringWithFormat:@"%@",@"通话中..."];
    }
}
        ...
        if (error) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if (error.code == EMErrorNetworkUnavailable) {
                    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"当前网络连接失败" delegate:nil cancelButtonTitle:@"ok" otherButtonTitles:nil, nil];
                    [alertView show];
                }
                else{
...
- (void)hangupAction
{
    ...
    [[VideoChatObject sharedInstance] hangupCallWithReason:EMCallEndReasonHangup];
}
...
- (void)rejectAction
{
    [_timeTimer invalidate];
    [self _stopRing];
    [[VideoChatObject sharedInstance] hangupCallWithReason:EMCallEndReasonDecline];
...
{
    self.statusLabel.text = @"连接建立完成";//    self.answerButton.enabled = YES;
}
...
- (void)stateToAnswered
{
    ...
    self.statusLabel.text = [NSString stringWithFormat:@"%@ %@",@"免提", connectStr];
